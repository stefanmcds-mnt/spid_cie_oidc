...
from pydantic import BaseModel, HttpUrl, ConfigDict, WrapValidator, model_validator
...
class OPMetadata(BaseModel):
    #model_config = ConfigDict(ignored_types=(('token_endpoint_auth_methods_supported','subject_types_supported')))
    ...
    '''
    token_endpoint_auth_methods_supported: Annotated[str, "private_key_jwt"]
    subject_types_supported: Annotated[str, "pairwise"]
    request_parameter_supported: Annotated[bool, True]
...
'''
    def validate(cls, values):
        ...
        for name, spec in cls.model_fields.items():
            if not isinstance(values[name], spec.annotation):
                values[name] = None
        for i in ("jwks_uri", "jwks", "signed_jwks_uri"):
                jwks_there = True
                break
        if not jwks_there:
            raise ValueError("one of signed_jwks_uri or jwks_uri or jwks must be set")
        return values
'''
...
class OPMetadataCie(OPMetadata):
    ...
    response_types_supported: Annotated[str, "code"]
    ...
    '''
    claims_parameter_supported: Annotated[bool, True]
    tls_client_certificate_bound_access_tokens: Annotated[bool,True]
    authorization_response_iss_parameter_supported: Annotated[bool, True]
